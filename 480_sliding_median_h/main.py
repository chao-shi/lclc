...
class MedianCalculator(object):
    ...
    def add_num(self, idx, num):
        if self.hmax.size() == 0 or num <= -self.hmax.first()[0]:
    ...
    def del_num(self, idx, num):
        # I did first mistake here
        # Tricky when num == self.hmax.first()[0], needs to compare index
        # to see if left or right
        if num < -self.hmax.first()[0]:
            self.hmax.remove(idx)
        elif num > -self.hmax.first()[0]:
            self.hmin.remove(idx)
        elif idx >= self.hmax.first()[1]:
            self.hmax.remove(idx)
        else:
            self.hmin.remove(idx)
        self.rebalance()
    ...
    def rebalance(self):
        if self.hmax.size() - self.hmin.size() == 2:
            num, idx = self.hmax.pop()
            self.hmin.add(idx, -num)
        elif self.hmax.size() - self.hmin.size() == -1:
